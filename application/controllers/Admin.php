...
class Admin extends CI_Controller
{
	...
	{	
		...
		{
			$data['resumes'] = $this->Job_model->get_all_cvs();
			...
			$this->load->view('admin/view-resumes', $data);
	...
	public function view_post($id = NULL)
    {
        if ($this->session->userdata('islogged_mko789') && $this->session->userdata('idtype_mko789') == 1) 
        {     
                $data['job'] = $this->Job_model->get_post_by_id($id);  
    ...
    public function view_employer($id = NULL)
    {
        if ($this->session->userdata('islogged_mko789') && $this->session->userdata('idtype_mko789') == 1) 
        {     
                $data['organization'] = $this->Employer_model->get_all_detail($id);  
                //die(var_dump($data));
                ...
                $this->load->view('template/header');
				$this->load->view('admin/admin-dashboard-nav');
				$this->load->view('admin/view-employer', $data);
				$this->load->view('template/footer');          
        } 
        else
        {
	...
	public function all_posts()
	{
		...
		$data['jobs'] = $this->Job_model->get_all_posts();
	...
	public function update_job()
	{
		...
		$output = array('error' => false);
		$this->form_validation->set_rules('old-post-img', 'Exist Image', 'xss_clean');
		$this->form_validation->set_rules('emp-name', 'Emplyer Name', 'xss_clean');		
		...
		else
		{
           ...
           if($output['error'] == false)
           {
				...
				if ($add == '') 				
					$job['post_image'] =  $_POST['old-post-img'];
				else
					$job['post_image'] = $add;			
				...
				$query_post = $this->Job_model->update_post($job);				

				if ($query_post) 
				{
					$output['message'] = 'Job Post Successfully Updated..';					
				}			
				else
				{
					$output['error'] = true;
					$output['message'] = 'Job Post Updating Failed, Something went Wrong!';
				}
			}
		}
		echo json_encode($output);	
	...
	public function update_employer()
	{
		...
		else
		{
			...
			if ($output['error'] == false) 
			{		          
	            $user['ref_org_id'] = $_POST['emp-ref-id'];	            
	            $user['org_name'] = $_POST['org-name'];
	            $user['username'] = $_POST['username'];
	            $user['org_telephone'] = $_POST['phone'];
				$user['email'] = $_POST['email'];		            
	            ...
	            if ($logo == '') 				
					$user['org_logo'] =  $_POST['old-logo'];
				else
					$user['org_logo'] = $logo;	            


	            $mail_status = $this->User_model->is_email_unique($_POST['email']);

	            $query_user = $this->Admin_model->update_employer($user, $mail_status);
